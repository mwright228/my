...
- **HAProxy 443 splitter:** Listens on port 443 and demultiplexes by content with zero decryption. TLS ClientHello matching the ShadowTLS decoy SNI routes to sing-box; all other TLS ClientHellos (your domain, carrier bug-hosts) route to the Nginx loopback TLS terminator (the WS-family transports); any non-TLS connection (raw Shadowsocks AEAD packets) goes to the shared SS-443 Xray inbound.
...
- **Dynamic Website Camouflage & Anti-Probing:** Root HTTP/HTTPS paths (`location /`) dynamically mirror an authentic legitimate website (`https://www.apple.com`) with subfilter rewrites and transparent caching, preventing active probing or censorship scanners from identifying a proxy engine.
- **Cloudflare WARP Smart Split-Outbound:** One-click integration with Cloudflare WARP WireGuard smart outbounds (`mubx-warp` / menu option `15`), automatically routing Netflix, ChatGPT/OpenAI, Disney+, Spotify, Hulu, and streaming services via residential/clean CDN egress while keeping general traffic direct.
...
- **Chameleon Universal Proxy & SSH Ingestion:** Dropbear SSH via direct ports (`2222`, `109`, `53`) and `mubx-chameleon` Universal Payload Engine (`8080`, `3128`, `8888`) — a zero-rejection HTTP CONNECT proxy purpose-built for mobile carrier bug-host payloads (split, delay_split, front/back inject), fully replacing legacy Squid. Unauthenticated payloads bridge strictly to local encrypted tunnel daemons (which enforce SSH/crypto credentials); outbound external relaying requires HTTP Proxy-Authorization: Basic, completely preventing open-relay abuse.
...
- **Kernel-Level Performance & Loss-Resilient BBR:** `mubx-tune` enables telecom-grade BBR + FQ, 64MB high-throughput TCP buffers, TCP Fast Open (client + server), MTU probing, and iptables TCPMSS clamping (`--clamp-mss-to-pmtu`) to eliminate packet fragmentation over jittery LTE/5G cellular networks.
- **Automated Self-Healing:** A weekly systemd timer renews Let's Encrypt certificates (restarting Hysteria/ZivPN on real renewals so they never serve an expired cert), updates GeoIP/GeoSite databases, enforces user bandwidth quotas, and revives any managed daemon that stopped running.
...
- **Mobile diagnostics:** `mubx-diagnose` reports public IPs, latency/loss, DNS, TCP reachability, path-MTU probes, kernel congestion control, and service health (`mubx-diagnose --chameleon` executes synthetic routing and open-relay protection checks).
...
`mubx-restart-failed`), a built-in self-update that pulls the latest
tree and re-renders everything in place (`13`, or `mubx-update`), and
subscription regeneration (`14`, or `mubx-sub`).
`svc-status` prints the reason under any service that is not running (port
conflicts, last journal line).

### Subscription URLs

Every user gets one subscription URL that imports all their nodes into any
client — files are static, generated by `mubx-sub` (menu 14) and refreshed
automatically on user add/remove and on every update:

```
https://<domain>/sub/<token>/index.txt      v2rayNG / NekoBox / Shadowrocket (base64 link list)
https://<domain>/sub/<token>/clash.yaml     Clash / Mihomo / Clash Verge
https://<domain>/sub/<token>/singbox.json   sing-box
https://<domain>/sub/<token>/links.txt      raw share links
```

Served HTTPS-only via the loopback TLS server; the token in the path is the
only credential.

### Optional features

- **Non-interactive install:** `MUBX_DOMAIN=vpn.example.com bash install.sh`
  skips the prompts. Add `MUBX_HY2_HOPPING=50000:51000` to also enable
  Hysteria2 port hopping in the same run.
- **Hysteria2 port hopping:** UDP `START:END` is DNAT'd onto 4433 (range must
  sit above 20000, clear of the ZivPN/UDPGW 6000-19999 set); links carry
  `mport=<domain>:<range>` and clients re-key across the range.
- **Shadowsocks 2022:** per-user `2022-blake3-aes-256-gcm` WS routes on
