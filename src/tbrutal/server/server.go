...
func (s *Server) handleUpgrade(w http.ResponseWriter, r *http.Request) {
    ...
    // Hijacking leaves a buffered reader behind. Feed that reader into the
    // T-Brutal session so bytes received together with the HTTP upgrade are
    // not discarded before the binary protocol parser starts.
    sessionConn := &prefixConn{Conn: conn, reader: buf}
    go NewSession(sessionConn, s.authStore, s.pacer).Handle()
}
