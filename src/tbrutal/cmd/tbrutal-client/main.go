...
func main() {
	...
	path := flag.String("path", "", "HTTP upgrade path (e.g. /tbrutal; empty string enables direct raw SNI mode)")
	raw := flag.Bool("raw", true, "Use direct raw TCP/TLS SNI mode without HTTP upgrade (like SSH SSL SNI)")
	...
	rateMbps := flag.Int("rate", 60, "Brutal pacing rate in Mbps (fixed wire-speed throughput)")
