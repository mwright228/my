...
func Java_id_my_mub_service_NativeCoreBridge_nativeStartTunnel(
    ...
    cfg := bridge.BridgeConfig{
        Protocol: safeGoString(cProtocol), ServerAddr: safeGoString(cServerAddr), SNI: safeGoString(cSNI),
        HostHeader: safeGoString(cHostHeader), Token: safeGoString(cToken), PoolSize: int(jPoolSize),
        RateMbps: int(jRateMbps), UseTLS: jUseTLS != 0, InsecureTLS: jInsecureTLS != 0,
        RawMode: jRawMode != 0, ObfsKey: safeGoString(cObfsKey), PortHopRange: safeGoString(cPortHopRange),
        DNSServer: safeGoString(cDNSServer), CustomPayload: safeGoString(cCustomPayload), SocksListenAddr: "127.0.0.1:0",
    }
...
func Java_id_my_mub_service_NativeCoreBridge_nativeStartTunRouter(env *C.JNIEnv, clazz C.jclass, jTunFd C.jint, jSocksPort C.jint, jDNSServer C.jstring) C.jboolean {
    ...
    cDNSServer := C.getStringUTFChars(env, jDNSServer)
    defer C.releaseStringUTFChars(env, jDNSServer, cDNSServer)
    if err := bridge.StartTunRouterWithDNS(int(jTunFd), int(jSocksPort), safeGoString(cDNSServer)); err != nil {
