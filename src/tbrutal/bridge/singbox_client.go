...
func (p *SingBoxPlatformBridge) AutoDetectInterfaceControl(fd int) error { if !ProtectSocket(fd) { return errors.New("failed to protect outbound socket from VPN routing loop") }; return nil }
...
func (c *SingBoxClient) Start() (int, error) {
    ...
    case strings.Contains(protoUpper, "SHADOWTLS") || strings.Contains(protoUpper, "STLS"):
        stlsPass := c.cfg.ObfsKey; if stlsPass == "" { stlsPass = c.cfg.Token }; cipher := "2022-blake3-aes-256-gcm"; if c.cfg.CustomPayload != "" && !strings.HasPrefix(c.cfg.CustomPayload, "/") { cipher = c.cfg.CustomPayload }; if stlsPass == "" || c.cfg.Token == "" { return 0, errors.New("ShadowTLS requires password and Shadowsocks credential") }
    ...
    case strings.Contains(protoUpper, "TUIC"):
        if c.cfg.Token == "" || c.cfg.ObfsKey == "" { return 0, errors.New("TUIC requires UUID and password") }; outboundsList = []map[string]any{{"type":"tuic","tag":"proxy","server":host,"server_port":serverPort,"uuid":c.cfg.Token,"password":c.cfg.ObfsKey,"congestion_control":"bbr","tls":map[string]any{"enabled":true,"server_name":effectiveSni,"insecure":insecure}}}
