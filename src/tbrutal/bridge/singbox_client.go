	...
	return func(network, address string, conn syscall.RawConn) error {
		...
		if err := conn.Control(func(fd uintptr) {
			if !ProtectSocket(int(fd)) {
				protectErr = errors.New("failed to protect outbound socket from VPN routing loop")
			}
		}); err != nil {
			return err
		}
...
func (p *SingBoxPlatformBridge) FindProcessInfo(ctx context.Context, network string, source netip.AddrPort, destination netip.AddrPort) (*process.Info, error) { return nil, nil }
...
func (c *SingBoxClient) Start() (int, error) {
	...
	if err != nil { return 0, fmt.Errorf("failed to allocate free port: %w", err) }
	...
	if err != nil || serverPort <= 0 { serverPort = 443 }

	effectiveSni := c.cfg.SNI
	if effectiveSni == "" { effectiveSni = host }
	effectiveHostHeader := c.cfg.HostHeader
	if effectiveHostHeader == "" { effectiveHostHeader = host }
	insecure := c.cfg.InsecureTLS

	path := c.cfg.Path
	if path == "" && c.cfg.CustomPayload != "" && strings.HasPrefix(c.cfg.CustomPayload, "/") { path = c.cfg.CustomPayload }
	...
	case strings.Contains(protoUpper, "SHADOWTLS") || strings.Contains(protoUpper, "STLS"):
		...
		if stlsPass == "" { stlsPass = c.cfg.Token }
		cipher := "2022-blake3-aes-256-gcm"
		if c.cfg.CustomPayload != "" && !strings.HasPrefix(c.cfg.CustomPayload, "/") { cipher = c.cfg.CustomPayload }
		outboundsList = []map[string]any{
			{"type": "shadowtls", "tag": "shadowtls-out", "server": host, "server_port": serverPort, "version": 3, "password": stlsPass,
				"tls": map[string]any{"enabled": true, "server_name": effectiveSni, "insecure": insecure}},
			{"type": "shadowsocks", "tag": "proxy", "server": host, "server_port": serverPort, "method": cipher, "password": c.cfg.Token, "detour": "shadowtls-out"},
		}
	...
	case strings.Contains(protoUpper, "HYSTERIA2") || strings.Contains(protoUpper, "HYSTERIA_2"):
		...
		if rate <= 0 { rate = 50 }
		outboundsList = []map[string]any{{
			"type": "hysteria2", "tag": "proxy", "server": host, "server_port": serverPort, "password": c.cfg.Token,
			"tls": map[string]any{"enabled": true, "server_name": effectiveSni, "insecure": insecure},
			"brutal": map[string]any{"enabled": true, "up_mbps": rate, "down_mbps": rate * 2},
		}}

	case strings.Contains(protoUpper, "TUIC"):
		outboundsList = []map[string]any{{
			"type": "tuic", "tag": "proxy", "server": host, "server_port": serverPort,
			"uuid": c.cfg.Token, "password": c.cfg.Token, "congestion_control": "bbr",
			"tls": map[string]any{"enabled": true, "server_name": effectiveSni, "insecure": insecure},
		}}
	...
	case protoUpper == "SHADOWSOCKS" || protoUpper == "SS" || strings.HasPrefix(protoUpper, "SS_") || strings.HasPrefix(protoUpper, "SHADOWSOCKS"):
		...
		if cipher == "" { cipher = "2022-blake3-aes-128-gcm" }
	...
	case strings.Contains(protoUpper, "TROJAN"):
		outboundsList = []map[string]any{{
			"type": "trojan", "tag": "proxy", "server": host, "server_port": serverPort, "password": c.cfg.Token,
			"tls": map[string]any{"enabled": true, "server_name": effectiveSni, "insecure": insecure},
			"transport": map[string]any{"type": "ws", "path": path, "headers": map[string]any{"Host": effectiveHostHeader}},
		}}

	case strings.Contains(protoUpper, "VMESS"):
		outboundsList = []map[string]any{{
			"type": "vmess", "tag": "proxy", "server": host, "server_port": serverPort, "uuid": c.cfg.Token,
			"alter_id": 0, "security": "auto",
			"transport": map[string]any{"type": "ws", "path": path, "headers": map[string]any{"Host": effectiveHostHeader}},
		}}

	case protoUpper == "VLESS_REALITY" || strings.Contains(protoUpper, "REALITY"):
		publicKey := strings.TrimSpace(c.cfg.ObfsKey)
		shortID := strings.TrimSpace(c.cfg.PortHopRange)
		if publicKey == "" || shortID == "" {
			return 0, errors.New("VLESS Reality requires profile-specific public key and short ID")
		}
		outboundsList = []map[string]any{{
			"type": "vless", "tag": "proxy", "server": host, "server_port": serverPort, "uuid": c.cfg.Token, "flow": "xtls-rprx-vision",
			"tls": map[string]any{
				"enabled": true, "server_name": effectiveSni, "insecure": insecure,
	...
	case protoUpper == "VLESS_TCP":
		outboundsList = []map[string]any{{
			"type": "vless", "tag": "proxy", "server": host, "server_port": serverPort, "uuid": c.cfg.Token,
			"tls": map[string]any{"enabled": true, "server_name": effectiveSni, "insecure": insecure,
				"utls": map[string]any{"enabled": true, "fingerprint": "chrome"}},
		}}
	...
	default:
		return 0, fmt.Errorf("unsupported sing-box protocol %q", c.cfg.Protocol)
	}
	...
	fullConfig := map[string]any{
		"log": map[string]any{"level": "info", "output": "stdout", "timestamp": true},
		"dns": map[string]any{
			"servers": []map[string]any{{"tag": "local", "address": "local", "detour": "direct"}},
			"strategy": "prefer_ipv4", "independent_cache": true,
		},
		"inbounds": []map[string]any{{"type": "mixed", "tag": "mixed-in", "listen": "127.0.0.1", "listen_port": port}},
	...
	if err != nil { return 0, fmt.Errorf("failed to serialize sing-box options: %w", err) }
	var options option.Options
	if err := options.UnmarshalJSON(configBytes); err != nil { return 0, fmt.Errorf("failed to parse sing-box options: %w", err) }

	ctx, cancel := context.WithCancel(context.Background())
	c.cancel = cancel
	platformBridge := &SingBoxPlatformBridge{}
	boxInstance, err := box.New(box.Options{Context: ctx, Options: options, PlatformInterface: platformBridge, PlatformLogWriter: nil})
	if err != nil { cancel(); return 0, fmt.Errorf("failed to initialize sing-box: %w", err) }
...
func (c *SingBoxClient) Stop() {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.instance != nil { _ = c.instance.Close(); c.instance = nil }
