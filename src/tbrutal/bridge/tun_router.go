...
func (r *TunRouter) handleUDP(packet []byte, ihl int, srcIP, dstIP net.IP) {
	if len(packet) < ihl+8 { return }
	h := packet[ihl:ihl+8]
	srcPort,dstPort := binary.BigEndian.Uint16(h[:2]),binary.BigEndian.Uint16(h[2:4])
	udpLen:=binary.BigEndian.Uint16(h[4:6])
	if udpLen<8 || len(packet)<ihl+int(udpLen) { return }
	payload:=append([]byte(nil),packet[ihl+8:ihl+int(udpLen)]...)
	if dstPort == 53 {
		select { case r.dnsSem <- struct{}{}: default: return }
	...
	r.handleGenericUDP(srcIP, dstIP, srcPort, dstPort, payload)
}
