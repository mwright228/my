...
object ProfileStore {
    ...
    fun setActiveProfileId(id: String) { prefs.edit().putString(KEY_ACTIVE_ID, id).apply() }

    fun getActiveProfile(): VpnProfile {
        val all = getAllProfiles(); val activeId = getActiveProfileId()
    ...
    fun getAllProfiles(): List<VpnProfile> {
        ...
        } catch (e: Exception) { throw IllegalStateException("Stored VPN profiles could not be decrypted or parsed", e) }
    }
    ...
    fun deleteProfile(id: String) {
        val current = getAllProfiles().toMutableList(); current.removeAll { it.id == id }; saveAll(current)
        if (getActiveProfileId() == id) setActiveProfileId(current.firstOrNull()?.id ?: "")
    }

    private fun saveAll(profiles: List<VpnProfile>) {
        val arr = JSONArray(); profiles.forEach { arr.put(profileToJson(it)) }
        val encrypted = requireNotNull(encrypt(arr.toString())) { "Unable to encrypt VPN profiles with Android Keystore" }
        check(prefs.edit().putString(KEY_PROFILES_ENCRYPTED, encrypted).remove(KEY_PROFILES_JSON).commit()) { "Unable to persist encrypted VPN profiles" }
    }

    private fun migratePlaintextProfiles() {
        val encrypted = prefs.getString(KEY_PROFILES_ENCRYPTED, null); val plaintext = prefs.getString(KEY_PROFILES_JSON, null)
        if (encrypted.isNullOrEmpty() && !plaintext.isNullOrEmpty()) {
            ...
            check(prefs.edit().putString(KEY_PROFILES_ENCRYPTED, sealed).remove(KEY_PROFILES_JSON).commit()) { "Unable to complete encrypted VPN profile migration" }
    ...
    private fun readEncryptedProfiles(): String? { val sealed = prefs.getString(KEY_PROFILES_ENCRYPTED, null); return if (sealed.isNullOrEmpty()) null else decrypt(sealed) }
    ...
    private fun decrypt(sealed: String): String? {
        val key = getKey() ?: return null
        return try {
            val parts = sealed.split(":", limit = 2); if (parts.size != 2) return null
            val iv = Base64.decode(parts[0], Base64.NO_WRAP); val ciphertext = Base64.decode(parts[1], Base64.NO_WRAP)
    ...
    private fun profileToJson(p: VpnProfile): JSONObject = JSONObject().apply {
        put("id",p.id); put("name",p.name); put("serverHost",p.serverHost); put("serverIp",p.serverIp); put("serverPort",p.serverPort)
    ...
    private fun profileFromJson(obj: JSONObject): VpnProfile {
        ...
        return VpnProfile(
            id=obj.optString("id",UUID.randomUUID().toString()), name=obj.optString("name","Unnamed Node"), serverHost=obj.optString("serverHost",""), serverIp=obj.optString("serverIp",""), serverPort=obj.optInt("serverPort",443),
