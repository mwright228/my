...
run apt-get install -y ca-certificates curl certbot dnsutils lsof psmisc git jq \
  ...
  ipset iptables iptables-persistent qrencode netcat-openbsd fail2ban python3 python3-systemd python3-uvloop \
...
# --- UDP 443 Ingress (Carrier Zero-Rating / Bug-Host Support) --------------
# Routes public UDP 443 to Hysteria 2 (4433) so carrier DPI zero-rating works
# directly on standard port 443 without conflicting with TCP 443 (HAProxy).
if [ -x /usr/local/bin/mubx-udp443 ]; then
  /usr/local/bin/mubx-udp443 hy2 >/dev/null 2>&1 || true
elif [ -x bin/mubx-udp443 ]; then
  bin/mubx-udp443 hy2 >/dev/null 2>&1 || true
fi
# --- Subscription files -----------------------------------------------------
...
install_success=1
printf '\n'
if [ -r lib/common.sh ]; then
  source lib/common.sh
  mubx_box_top "MUB-X TELECOM ENGINE" "ONLINE"
  mubx_kvrow "Gateway Host"   "$DOMAIN"
  mubx_kvrow "Control Panel"  "menu"
  mubx_kvrow "Client Links"   "link-gen"
  mubx_kvrow "Service Matrix" "svc-status"
  mubx_kvrow "User Store"     "mubx-users"
  mubx_box_bot
else
  echo "[✓] MUB-X installed successfully on $DOMAIN."
fi
printf '\n'
